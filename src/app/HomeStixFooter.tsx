...
import { exportAllToStix } from "@/app/exportAllToStix";
...
export default function HomeStixFooter() {
    const [isPending, startTransition] = useTransition();
    const [status, setStatus] = useState<"idle" | "ok" | "error">("idle");
    const [exportInfo, setExportInfo] = useState<{ binaries: number; manifestPath: string } | null>(null);

    const handleClick = () => {
        setStatus("idle");
        startTransition(async () => {
            try {
                const res = await exportAllToStix();
                setExportInfo(res);
                setStatus("ok");
            } catch (e) {
                console.error(e);
...
"
            ...
            >
                ...
                <span>{isPending ? "Exportando todo..." : "Exportar todo a STIX"}</span>
            </button>

            {status === "ok" && exportInfo && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-[#F5F4CB] text-emerald-900 rounded-xl shadow-2xl px-6 py-5 max-w-sm w-full mx-4">
                        ...
                        <p className="text-sm mb-4">
                            Se han exportado <strong>{exportInfo.binaries}</strong> binarios a STIX.
                        </p>
                        <div className="flex flex-wrap gap-3 justify-end">
                            <a
                                href={exportInfo.manifestPath}
                                download
            ...
            {status === "error" && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                    <div className="bg-[#F5F4CB] text-red-800 rounded-xl shadow-2xl px-6 py-5 max-w-sm w-full mx-4">
                        ...
                        <p className="text-sm mb-4">
                            Hubo un problema al exportar todas las reglas a STIX. Revisa los logs del servidor.
                        </p>
